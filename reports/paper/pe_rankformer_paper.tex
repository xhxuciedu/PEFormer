...
\title{\textbf{PE-RankFormer: metric-matched ordinal\\
supervision and state-space sequence mixing\\
...
--- the deployment-relevant task --- absolute performance is far lower for both models
...
the first tempers the headline and the second strengthens the comparison. On the
retrieval metric a user cares about, the model identifies a target's best design in
$36.7\%$ of cases against the baseline's $22.4\%$; the efficiency actually obtained from
its top pick, however, improves by only $+0.015$. It is $+0.0804$ on the more
...
\textbf{The absolute level falls a long way.} Within a protospacer group --- the actual
design problem --- our model reaches $\rhoS = 0.6356$, not $0.9079$. Choosing among
pegRNAs at a fixed locus is a substantially harder task than the headline number
suggests, and we regard $0.64$, not $0.91$, as the honest statement of what the model
does for a user with one target in hand. The pooled figure should not be read as a
design-ranking capability.
...
target, ahead on $78.4\%$ of the 670 scorable targets. Removing the locus effect removes
signal that \emph{both} models exploit equally, and what remains is the part attributable
to modelling. Pooling therefore inflates the absolute level while making the reported
\emph{difference} conservative.
...
therefore report per-target retrieval and regret metrics
(Table~\ref{tab:utility}), computed within a protospacer group and averaged over groups,
against OptiPrime and against selecting a design at random.

The ranking metrics and the outcome metrics disagree, and the disagreement is the
result. PE-RankFormer identifies the single best design at a target far more often than
the comparator --- precision@1 of $0.367$ against $0.224$, a $64\%$ relative gain --- and
its regret, the efficiency forgone by taking its first pick instead of the target's best
design, falls from $0.049$ to $0.033$. But the efficiency actually obtained from the top
pick rises only $+0.015$, and the probability that the top pick works at all
($\geq5\%$ efficiency) rises $+0.011$ with an interval spanning zero. Both models'
first picks are usually already viable, so the gain is concentrated in finding the
\emph{best} design rather than in avoiding a failed one. A reader deciding whether to
adopt the model should weigh $+0.143$ precision@1 against $+0.011$ on "does the first
pick work", and we give both rather than the flattering one.
...
them than have them found. Evaluated \emph{within} a target --- the design problem a
user actually faces --- both models score far lower than the pooled figure implies
($0.6356$ against $0.5472$), though the margin there more than doubles to $+0.0884$;
pooling flatters the level while making the difference conservative. On the
...
Kim. And the gain concentrates in identifying a target's \emph{best} design
(precision@1 $0.367$ against $0.224$) rather than in avoiding a failed one, where the
improvement is $+0.011$ and not statistically distinguishable from zero.
...
Progress will require richer context annotation, or an independent dataset, rather than
...
\paragraph{Reproducing the reported results.} A single command,
\path{reproduce/run_all.sh}, regenerates every number, table and figure in this paper
...
every comparison in this paper can be checked without access to the training data.
