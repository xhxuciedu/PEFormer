...
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item Patience equals the epoch budget, i.e.\ no early stopping. Across 25
official-fold runs the best epoch had median 21 and maximum 28, with 36\% peaking in
the final five epochs; validation Spearman improves after long plateaus on this
problem, and simulating patience-5 selection costs up to $0.0012$ on individual runs.
...
\midrule
Final vs.\ OptiPrime   & +0.0389 & $[+0.0286,\,+0.0498]$ & 1.000 & $<0.0002$ \\
...
\textbf{architecture} $=+0.0192$; \textbf{interaction} $=-0.0043$. Architecture
contributes roughly twice what the objective does, and the negative interaction
indicates the two mechanisms partly capture the same improvement.
...
recipe, so the main effects are reliable at the $\pm0.002$ resolution of the
development evaluation; the interaction term is most exposed to that mismatch.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\centering
\begin{threeparttable}
\caption{Effect of monotone calibration on held-out predictions.}
\label{tab:calib}
\begin{tabular}{l S[table-format=1.4] S[table-format=1.4] S[table-format=1.4] S[table-format=1.4]}
\toprule
Output & {Spearman $\rhoS$} & {Pearson $r$} & {MAE} & {RMSE} \\
\midrule
Rank average (uncalibrated)     & 0.9079 & 0.7278 & 0.3869 & 0.4343 \\
\textbf{+ isotonic calibration} & \bfseries 0.9079 & \bfseries 0.8637 & \bfseries 0.0478 & \bfseries 0.0912 \\
\midrule
Change & {$+0.00002$} & {$+0.1359$} & {$-0.3391$} & {$-0.3431$} \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
...
\item MAE and RMSE are in units of editing efficiency (fraction of alleles edited).
For the uncalibrated output they are not meaningful in that unit, since a rank average
lies on an arbitrary $[0,1]$ scale; they are shown to quantify what calibration
recovers.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\toprule
Scope & {Rows} & {Model $\rhoS$} & {Ceiling $\rhoS$} & {Gap} \\
\midrule
All Kim rows      & 19747 & 0.7869 & 0.9026 & +0.1157 \\
Rows with $y>0$   &  9713 & 0.8465 & 0.9489 & +0.1024 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
...
target site are identical (\S\ref{sec:ceiling}).
...
\item The second row shows the gap is not merely an artifact of the zero block:
restricting to rows that actually edit still leaves more than $+0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}
