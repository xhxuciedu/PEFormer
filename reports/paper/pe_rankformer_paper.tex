...
\title{\textbf{Decision-aligned evaluation reveals a disconnect\\
between prime-editing efficiency prediction\\
and pegRNA selection}}
...
protospacer-clustered bootstrap resamples, $p < 0.0002$).

That pooled figure conflates which locus is easy to edit with which pegRNA is best at a
locus, and only the second is the design problem. Evaluated \emph{within} a protospacer,
absolute performance is far lower for both models
($\rho = 0.6356$ against $0.5472$) while the margin more than doubles to $+0.0884$
(95\% CI $[+0.0761,+0.1007]$, ahead on $78.4\%$ of 670 targets). We report both, because
the first tempers the headline and the second strengthens the comparison. However,
protospacer groups can still mix intended alleles and experimental contexts. On
1{,}463 informative fixed-allele/context source decisions, achieved top-choice
efficiency is 0.1364 versus 0.1272. On an external library the zero-shot advantage
reverses: 0.0363 versus 0.0369 across all 30{,}475 eligible decisions. The source
Spearman margin is $+0.0804$ on the more
...
remaining headroom (held-out gap $+0.1040$, 95\% CI $[+0.0529,+0.1204]$), and we report a benchmark caveat: predictors that emit
...
Retrospective low-label adaptation on a separate library improves target selection,
but stronger optimization controls and acquisition replication do not establish
superiority over released OptiPrime. Source-anchored adaptation alone does not
reliably preserve source choices; decision-preservation training improves retention
at an explicit target-utility cost under constrained selection. This distinguishes
benchmark prediction gains from a validated, transferable design-selection method.
\end{abstract}
...
keep thresholds strictly increasing. $K=20$ yields \textbf{17} distinct thresholds in
every one of the twenty official-fold checkpoints that constitute the reported system,
and 18 on the matched development folds, whose different training subset leaves one
fewer duplicate at the bottom of the distribution. Figure~\ref{fig:arch}C shows the
...
Ordinal thresholds $K-1$         & 17            & Selection            & best validation $\rhoS$ \\
...
to remain strictly increasing. $K{=}20$ yields 17 distinct values in all twenty
official-fold checkpoints, because the target has a large point mass at zero; the
matched development folds retain 18. Counts were read back from the checkpoints rather
than assumed (\path{revision/audit_00_consistency.py}).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The primary metric is Spearman's rank correlation $\rho$ \citep{spearman1904} between
predicted and observed efficiency,
...
All model comparisons use a \textbf{paired protospacer-clustered bootstrap}
\citep{efron1993,cameron2008} with 5{,}000 resamples. Protospacers, not rows, are
resampled: many designs share a protospacer, so a row-level bootstrap would treat
correlated observations as independent and materially understate the intervals. That is
not hypothetical here: clustering instead on a finer target-site key narrows the
interval on the within-condition margin from $[+0.0345,+0.0670]$ to
$[+0.0451,+0.0564]$, a spurious halving. Both models are evaluated on identical rows in each resample.
...
constructs, i.e.\ \emph{different molecules}, and the structured 3$'$ extension is known
to change efficiency substantially \citep{nelson2022}. Treating them as replicates gives
...
\subsection{What the pooled metric measures, and what it hides}
\label{sec:stratified}

Pooled Spearman on the held-out set ($0.9079$) exceeds both partition scores (Liu
$0.8585$, Kim $0.8124$). That is arithmetically possible because the two studies differ
in efficiency level, so between-study variance contributes signal, and it is a warning:
the pooled figure conflates two questions. \emph{Which locus is easy to edit} and
\emph{which pegRNA is best at this locus} are different problems, and only the second is
what a predictor is deployed to answer. The model is also given source study as one of
its seven context fields, which a reader may reasonably regard as a dataset identifier
rather than a biological covariate.

We therefore report the comparison at three levels of stratification
(Table~\ref{tab:stratified}), and the two findings point in opposite directions.

\textbf{The absolute level falls a long way.} Within a protospacer group our model
reaches $\rhoS = 0.6356$, not $0.9079$. This removes some between-locus signal, but
protospacer groups can still combine different intended alleles and experimental
conditions. The statistic is therefore an intermediate stratified comparison, not
a pure measure of the fixed-edit design problem. The deployment-aligned grouping
is evaluated in \S\ref{sec:utility}.

\textbf{The margin more than doubles.} Against the same rows, the advantage over
OptiPrime grows from $+0.0389$ pooled to $+0.0506$ within condition to $+0.0884$ within
target, ahead on $78.4\%$ of the 670 scorable targets. Removing between-protospacer
variation increases the observed margin, although remaining within-group
allele/context variation prevents attributing the entire difference to choosing
alternative designs for one fixed edit. Pooling inflates the absolute level while
reducing this particular stratified difference.

\input{tables/tab_stratified}

\subsection{What the model is worth at the bench}
\label{sec:utility}

Rank correlation measures how well a whole list is ordered. A user asks something
narrower: if I synthesise the design this model puts first, do I get an edit? We
therefore report retrieval and regret within one intended allele and experimental
context, averaging repeated measurements of the same design before selecting
(Table~\ref{tab:utility}). Historical protospacer-only groups remain as a
descriptive comparison, not the deployment task.

On 1{,}463 informative source decisions, PE-RankFormer's achieved top-choice
efficiency is 0.1364 versus 0.1272, a difference of 0.00915
(95\% CI [0.00633, 0.01215]). Best-design hit rate is 0.7423 versus 0.6535;
these groups are shallow, and none contains five distinct candidates. They cannot
establish performance at deeper library-selection budgets.

The external library reverses the zero-shot utility comparison. Across all
30{,}475 eligible fixed-allele groups, including all-zero groups, achieved
efficiency is approximately 0.0363 for our ordinal--state-space ensemble versus
0.0369 for OptiPrime. On the 24{,}668 informative groups shown in panel C,
the corresponding values are 0.0448 versus 0.0456, with difference $-0.00078$
(95\% CI [$-0.00111$, $-0.00046$]). The source final ensemble and external
ordinal--state-space ensemble are different configurations. Better source ranking
and higher external pooled correlation therefore do not establish better external
top-choice utility; target adaptation is evaluated separately below.

\input{tables/tab_utility}

\input{sections/adaptation_followup}

\subsection{Tie-robust and decomposed metrics}
\label{sec:tierobust}

Section~\ref{sec:ties} shows that Spearman's tie convention rewards a predictor that
emits ties on this zero-inflated target, by $+0.013$ to $+0.016$ irrespective of quality.
That makes the tie structure of the two prediction vectors a precondition for the
comparison rather than a footnote to it, and it is satisfied: both are effectively
tie-free, with 20{,}509 and 20{,}504 distinct values over 20{,}509 rows.
Kendall $\tau_b$ \citep{kendall1945}, which corrects for ties in both variables, agrees
with Spearman in direction and gives a larger margin ($+0.0547$ against $+0.0389$).

Decomposing the target separates two abilities the pooled metric conflates
(Table~\ref{tab:tierobust}). On \emph{detection} --- telling an edit from no edit at all
--- AUROC improves by $+0.0292$. On \emph{quantification} --- ordering only the rows that
do edit --- Spearman improves by $+0.0469$. The advantage is therefore not an artifact of
the zero block: it is present in both components, and larger in the harder one. Liu is
again the weakest partition on every metric, with an AUROC interval spanning zero.

\input{tables/tab_tierobust}

\subsection{Design-and-condition twins do not carry the result}
\label{sec:leakage}

Because the official split partitions protospacers rather than experiments, a held-out
row can in principle have an exact twin in training --- a row matching it on all sixteen
design and condition covariates \emph{and} the target site. We identified every such row
and recomputed the comparison without them (Table~\ref{tab:leakage}).

There are 196, or $1.0\%$ of the held-out set. Removing them moves the headline from
$0.9079$ to $0.9075$ and the margin from $+0.0389$ to $+0.0385$. The exposure is
therefore real and negligible, and we report it as a measurement rather than, as an
earlier draft did, as an unquantified caveat.
...
\paragraph{A weighting-matched comparison.} We trained PE-RankFormer under the
comparator's own per-row weights, against matched controls on all three development
folds --- same seed, same fold, same recipe, same code state, so the weighting is the
only difference. Uniform weighting wins on every fold: $+0.0148$, $+0.0108$, $+0.0151$,
mean $\mathbf{+0.0136}$, same sign on 3/3, which clears the promotion bar this paper
applies to its own candidate mechanisms (\S\ref{sec:resolution}).

The loss is where the mechanism predicts it, and only there
(Table~\ref{tab:weightexpt}). On the Kim partition, adopting the comparator's weights
costs $+0.0247$ (95\% CI $[+0.0144,+0.0355]$); on Liu it costs $+0.0063$ with an interval
spanning zero ($[-0.0052,+0.0179]$), which is expected because Liu's effective share
\emph{rises} slightly under those weights. Two consequences follow, and we state both.

Roughly a third of the $+0.0803$ Kim margin is attributable to the weighting choice
rather than to architecture or objective. And the $+0.0220$ Liu margin is
weighting-independent --- making it simultaneously the most conservative statement of
our result, the one measured on the surface the comparator itself reports, and the one
that most cleanly isolates the modelling contribution.

\begin{table}[H]
\centering
\begin{threeparttable}
\caption{Effect of training under the comparator's own per-row weights.}
\label{tab:weightexpt}
\begin{tabular}{l S[table-format=1.4] S[table-format=1.4] S[table-format=+1.4] c}
\toprule
& {OptiPrime weights} & {Uniform} & {$\Delta\rhoS$} & {95\% CI} \\
\midrule
\multicolumn{5}{l}{\emph{Development folds, matched controls}} \\
\quad fold 0 & 0.8830 & 0.8978 & +0.0148 & {---} \\
\quad fold 1 & 0.8905 & 0.9013 & +0.0108 & {---} \\
\quad fold 2 & 0.8814 & 0.8965 & +0.0151 & {---} \\
\quad \textbf{mean of three folds} & \bfseries 0.8850 & \bfseries 0.8985 & {$\mathbf{+0.0136}$} & {---} \\
\midrule
\multicolumn{5}{l}{\emph{Fold 0, split by source study}} \\
\quad Kim partition & 0.7389 & 0.7636 & +0.0247 & $[+0.0144,+0.0355]$ \\
\quad Liu partition & 0.8492 & 0.8556 & +0.0063 & $[-0.0052,+0.0179]$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item Positive $\Delta\rhoS$ means uniform weighting is better. Each pair shares seed,
fold, recipe and code state, so the weighting is the only difference.
\item Intervals are protospacer-clustered bootstraps over 5{,}000 resamples. Per-fold
rows are single matched differences and carry no interval; their evidence is the
same-sign replication across three folds.
\item The Liu interval spanning zero is the prediction, not a failure to detect: Liu's
effective share of the training gradient \emph{rises} from $22.0\%$ to $24.8\%$ under
the comparator's weights, so no loss is expected there.
\item Reproduce with \path{revision/task_2_round9_wave_results.py} and
\path{revision/task_1_8_weighting_by_partition.py}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
An absolute error of $0.0478$ is only interpretable against a floor, because the target
is small-valued: a constant predictor is already close in absolute terms. The best
trivial predictor --- a constant at the training median --- reaches MAE $0.1227$, so the
calibrated model represents a $61\%$ reduction against that floor and the figure reflects
skill rather than the scale of the target (Table~\ref{tab:calibfloor}).

The same table records a limitation of the calibration that matters for design. The
isotonic map \emph{over-predicts its own tail}: in the model's top-scoring $1\%$ it
predicts $0.799$ against $0.733$ observed. Isotonic regression has the least data there
and is monotone-constrained, so the final bin absorbs the whole upper range. Ranking is
unaffected, but a user reading an absolute prediction in the tail --- which is exactly
where a design decision is most likely to rest on the number rather than the rank ---
should treat it as optimistic \citep{guo2017}.

\input{tables/tab_calibfloor}
...
development folds the gap is $+0.1146$ (mean of three); on the held-out set the ceiling is $0.9164$ against a model score of $0.8124$, a gap of $+0.1040$.
Restricting to rows that actually edit leaves $+0.1046$ and $+0.0934$ respectively, so
the gap is not merely the zero block. The conclusion is the same on every surface and
does not depend on which is chosen: roughly a tenth of a Spearman unit of reducible error
remains on the benchmark's largest partition.

The ceiling itself carries real uncertainty, which an earlier version of this analysis
did not report. Resampling the 649 replicate groups gives a 95\% interval on the
held-out gap of $[+0.0529,+0.1204]$ (Table~\ref{tab:ceiling}). The lower bound stays
clear of zero on all four surfaces, so the qualitative claim --- the benchmark is not
saturated --- survives; what the interval removes is the precision with which the
headroom can be quoted. ``Roughly $+0.10$'' is supportable and a third decimal place is
not. Noise in the estimator itself, holding the group set fixed, is an order of magnitude
smaller than the group-resampling interval, so the binding constraint is that the corpus
contains only 649 clean replicate groups.

\input{tables/tab_ceiling_uncertainty}
...
\textbf{(C)} Both corrections described in \S\ref{sec:ceiling} are load-bearing, for
three estimators of the same ceiling. \emph{naive key}: grouping on six covariates,
which merges epegRNA with plain-pegRNA constructs and returns a ceiling essentially
equal to the model's own score (dashed line), wrongly indicating saturation.
\emph{exact zeros}: the full seventeen-covariate key but treating an observed zero as
noiseless, which over-estimates the ceiling. \emph{censored zeros}: the full key with
zeros treated as censored, which is the estimate we use. Panel~C is shown for
development fold 0, as is panel~A.}
\label{fig:ceiling}
\end{figure}
...
\midrule
\multirow{4}{*}{Parameterisation}
 & rank-consistent CORAL head          & +0.0005 & null \\
 & monotonicity penalty                & +0.0007 & null \\
 & hurdle (zero-inflation) head        & -0.0022 & harmful \\
 & censoring-aware ordinal loss        & -0.0079 & control beat it \\
\midrule
\multirow{7}{*}{Architecture}
 & selective (Mamba-style) SSM         & +0.0031 & below threshold \\
 & context-conditioned low-rank map    & +0.0000 & null vs.\ own control \\
 & cross-attention blocks 2$\to$4      & -0.0012 & harmful, $+29\%$ params \\
...
\item \textbf{These entries are bounds, not zeroes.} At the resolution established in
\S\ref{sec:resolution} a single-fold null excludes effects larger than
$\approx\pm0.008$ and a three-fold null excludes $\approx\pm0.005$ at $80\%$ power, so
the defensible statement is that no intervention produced a replicated gain above
$\approx0.005$ --- not that these mechanisms have no effect. Under Holm correction
\citep{holm1979} across the twenty entries only the three with $|\Delta\rhoS|>0.01$
survive, all of which the table already marks as harmful rather than null.
\item Two round-9 entries are informative about method rather than architecture. The
context-conditioned low-rank map is the instrument \S\ref{sec:domain} argues FiLM lacks
--- a non-diagonal, context-dependent transform --- and it scored identically to a
capacity-matched control that cannot see context ($0.8969$ both), which closes the
context-conditioning family by direct test rather than by inference. The
censoring-aware loss drops the ordinal threshold terms a measured zero cannot inform;
it was beaten by a shuffle control dropping the same \emph{number} of terms at random
($-0.0091$), so the zero boundary carries ranking signal that the correction discards.
...
sequences. Long-convolution and state-space architectures have been applied to genomic
sequence chiefly to reach very long context --- HyenaDNA extends to $10^6$ nucleotides
\citep{nguyen2023} --- which is the opposite regime from ours. Our sequences are 100 and
90 tokens, so the benefit we observe cannot be about context length and must be about
the inductive bias itself, which is the more transferable claim of the two. We note one negative constraint from our results: naive
...
Given the headroom is real, the natural question is why roughly seventy-five subsequent
...
\item 196 held-out rows ($1.0\%$) have an exact design-and-condition twin in training,
because the split partitions protospacers rather than experiments. Measured rather than
assumed: removing them moves the headline by $0.0004$ (\S\ref{sec:leakage}), so this is
a bounded non-issue rather than an open caveat.
...
the comparison.
\item\label{sec:limitations-baselines} The original architecture comparisons use a
single benchmark and one external comparator plus the feature baseline of
\S\ref{sec:gbm}. The separate-library adaptation study
(\S\ref{sec:adaptation-followup}) is retrospective development within a
source-adjacent study family, not independent confirmation. Its optimizer-seed
replications use one acquisition subset, and its per-fit label budgets do not
include all labels used in research-level model development. Independent,
adequately powered selection validation is not yet available. We attempted two
further baselines, PRIDICT2.0 and DeepPrime-FT, and
...
predictor of prime-editing efficiency on that predictor's own benchmark, reaching
Spearman $0.9079$ against $0.8690$ ($\Delta=+0.0389$, ahead in all 5{,}000
protospacer-clustered bootstrap resamples). A factorial analysis attributes roughly
two-thirds of the gain to replacing self-attention with a bidirectional state-space
sequence mixer and one-third to an ordinal outcome head matched to the rank-correlation
metric and to the target's zero inflation; a single model containing both matches a
five-member ensemble, and the same model also leads on absolute error once calibrated
($0.0478$ against $0.0590$ MAE, against a trivial floor of $0.1227$).

Three qualifications belong in any summary of that result, and we would rather state
them than have them found. Evaluated within historical protospacer groups, both
models score far lower than the pooled figure implies
($0.6356$ against $0.5472$), though the margin there more than doubles to $+0.0884$;
these groups still mix alleles and conditions, so fixed-edit utility requires a
separate evaluation. On the
partition the comparator itself reports, the margin is $+0.0220$ at $p=0.028$, and the
much larger Kim margin is substantially explained by a training-weight asymmetry rather
than by modelling: the comparator's own released weights allocate the Kim study $2.0\%$
of its training gradient while that study supplies $55.3\%$ of the evaluation rows, and
adopting those weights costs us $+0.0136$ across three folds, essentially all of it on
Kim. Finally, fixed-allele source selection improves (0.1364 versus 0.1272 achieved
efficiency), whereas external-library zero-shot selection does not (0.0363 versus
0.0369 across all eligible decisions). The latter is a direct limit on transfer
of the benchmark advantage.

The benchmark is not saturated: replicate analysis indicates roughly $+0.10$ Spearman of
reducible error remains on its largest partition (95\% CI $[+0.05,+0.12]$). Our evidence
suggests the obstacle is neither capacity nor supervision --- around seventy-five
controlled experiments across architecture, objective, parameterisation and training
scheme have failed to close it, including a context-conditioned non-diagonal transform
built specifically to supply the capacity our own diagnostics said was missing, which
scored identically to a control that cannot see context at all. What remains is most
likely the absence of covariates describing how experimental context reorders designs,
and no objective over a categorical label can recover biology the label does not encode.
These original-benchmark results motivate richer context annotation and independent
data rather than an unrestricted architecture search. They do not rule out gains
from target-domain supervision: the separate adaptation experiments improve target
selection at the historical full label budget, but expose source-retention costs.
The low-label studies have not established a reliable external-library advantage
over released OptiPrime. Stronger optimization and acquisition replication remove
the simple claim that anchoring alone improves the target--source trade-off;
explicit choice preservation improves source robustness but does not establish
joint target superiority and retention. An independently validated
selection method remains an objective, not a conclusion of the present work.

% =====================================================================
\section*{Data and code availability}

\paragraph{Code.} All model code, configurations, per-run metadata, the complete research
log including every negative result, and the frozen model specification are in the
project repository, together with 232 unit tests.

\paragraph{Reproducing the original benchmark results.} A single command,
\path{reproduce/run_all.sh}, regenerates the original benchmark numbers, tables and figures
from the frozen prediction files, in roughly 25 minutes on one CPU and with no GPU. It
also runs \path{reproduce/verify_manuscript.py}, which checks 44 numeric claims made in
the body text against the artifacts that produced them and fails if any has drifted; and
\path{revision/make_paper_tables.py}, which generates every result table from the
analysis JSON so that no tabulated number is stored twice. Each analysis script records
its git commit, random seed, and a SHA-256 prefix of every input file into its output.
Retraining from scratch is documented separately in \path{reproduce/README.md} and takes
approximately 60 GPU-hours.

\paragraph{Adaptation follow-up.} The v3 and v3.1 adaptation analyses are separate
from that historical reproduction command. Their execution protocols, total-label
manifests, checkpoint-selection rules, full input/output fingerprints and result
ledgers are documented in \path{explore_v3/README.md} and
\path{explore_v3_1/README.md}. Their private-data-dependent training and cached
evaluations are not claimed to be reproducible from the original public benchmark
prediction files alone.

\paragraph{Data.} The corpus is a harmonisation of measurements from three previously
published studies \citep{kim2021,mathis2023,yu2023,mathis2024}, assembled by the authors
of the comparator method \citep{hsu2026} and shared with us privately. We do not
redistribute it: the decision to release that harmonisation is theirs, not ours. The
repository contains the build script that reconstructs the processed corpus from those
files and asserts its 297{,}962-row training count, so a reader who obtains them from
the original authors can rebuild our inputs exactly and verify the dataset hash our runs
record. The frozen per-row prediction vectors for both models on the held-out set
\emph{are} included, which is what the re-analyses in \S\ref{sec:stratified}--
\S\ref{sec:leakage} and Tables~\ref{tab:stratified}--\ref{tab:ceiling} operate on, so
the original benchmark comparisons can be checked without access to the training data.

We regard the absence of a public corpus as the most significant limitation on the
reproducibility of this work, and we note that it is not ours to remove.
...
Cameron AC, Gelbach JB, Miller DL.
Bootstrap-based improvements for inference with clustered errors.
\emph{The Review of Economics and Statistics} \textbf{90}(3), 414--427 (2008).
...
\emph{Scandinavian Journal of Statistics} \textbf{6}(2), 65--70 (1979).

\bibitem[J\"arvelin \& Kek\"al\"ainen(2002)]{jarvelin2002}
J\"arvelin K, Kek\"al\"ainen J.
Cumulated gain-based evaluation of IR techniques.
\emph{ACM Transactions on Information Systems} \textbf{20}(4), 422--446 (2002).

\bibitem[Kendall(1945)]{kendall1945}
Kendall MG.
The treatment of ties in ranking problems.
\emph{Biometrika} \textbf{33}(3), 239--251 (1945).
...
\url{https://doi.org/10.1038/s41587-024-02268-2}.

\bibitem[Nelson et al.(2022)]{nelson2022}
Nelson JW, Randolph PB, Shen SP, et al.
Engineered pegRNAs improve prime editing efficiency.
\emph{Nature Biotechnology} \textbf{40}, 402--410 (2022).
