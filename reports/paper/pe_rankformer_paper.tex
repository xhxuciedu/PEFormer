...
across successive model generations. All models were trained on the identical corpus
and cross-validation splits and evaluated on identical rows; the dotted line marks the
baseline's overall performance. The advantage is consistently larger on the Kim
partition, which spans more cell lines and editing systems.
...
line marks zero.
\textbf{(C)} Held-out performance broken down by experimental condition
($n\geq300$), coloured by source study. Performance spans $0.21$ between the weakest
and strongest conditions, and the weakest are those with the highest fraction of
exactly-zero measurements (cf.\ Figure~\ref{fig:ceiling}B).}
\label{fig:bench}
\end{figure}
...
group means). Ordinal-head models separate cleanly from simplex-head models, i.e.\ they
make \emph{different errors}, which is what determines a member's value in an ensemble
(\S\ref{sec:ensemble}). Lower is more complementary.}
\label{fig:fact}
\end{figure}
...
\textbf{(C)} Metric comparison. Because $g$ is non-decreasing it inverts no pair, so
Spearman is unchanged, while Pearson correlation rises and absolute error falls roughly
eight-fold.}
\label{fig:calib}
\end{figure}
...
condition, ordered by zero-inflation; the annotation gives the gap. Headroom is present
in every condition.
