# PE-RankFormer: pegRNA ranking and selection

Start with [main.pdf](main.pdf) or edit [main.tex](main.tex). The manuscript presents PE-RankFormer as an edit-aware ranking method: architecture first, rank-based comparisons against OptiPrime next, then source selection and target-supervised external selection. Detailed controls, unsuccessful extensions, fixed-recipe v3 work and acquisition-replicated v3.1 work are retained in the supplement. Earlier manuscripts under `reports/paper/` are preserved unchanged.

The 14 September writing revision changes presentation, not research results. Source and external zero-shot ranking favor PE-RankFormer; source selection also improves. External selection superiority is demonstrated after target-library fine-tuning against **unadapted released OptiPrime**, not in zero-shot selection or the low-label replication. The within-library adaptation test is retrospective, not independent-study confirmation.
...
- `figures/`: five generated quantitative figures, PDF and PNG. The main paper leads with a vector architecture diagram, a ranking comparison and a selection figure; three diagnostic figures appear in the supplement.
- `tables/`: eight automatically generated result tables, including a main-text three-seed external selection comparison with paired confidence intervals. Additional design, source-ranking and supplementary accounting tables are in the section sources.
...
Upload the contents to a LaTeX editor with `main.tex` as the root document. Line numbers are enabled for review; remove `\linenumbers` to disable them. Selection figures use percentage or percentage-point axes; numerical selection tables use efficiency fractions. Ranking metrics are dimensionless.
...
The reviewed research baseline is commit `b55a14462a4b7e217041ffd763e90edf83f8860e`. The writing revision does not retrain models or change recorded outcomes. A final archival release identifier must be set after author approval.
